...
struct mnt_pt_stat
{
    int64_t cp_bytes_read;
    int64_t cp_bytes_written;
};
...
int main(int argc, char **argv)
{
    ...
    struct mnt_pt_stat* mnt_pts_stats;
    time_t tmp_time = 0;
    int64_t run_time = 0;
    char *token;
    char *save;
    ...
    int empty_mods = 0;
    char *mod_buf;
    int mnt_pts_stats_index;
    struct darshan_posix_file *pfile;
    struct darshan_stdio_file *sfile;
    ...
    /* note: we allocate one extra element to be the catch-all for "unknown"
     * mounts
     */
    mnt_pts_stats = calloc(mount_count+1, sizeof(*mnt_pts_stats));
    assert(mnt_pts_stats);
    ...
    for(i=0; i<DARSHAN_MAX_MODS; i++)
    {
        ...
        /* skip anything that's not posix or stdio for this parser use case */
        if(i != DARSHAN_POSIX_MOD && i != DARSHAN_STDIO_MOD)
            continue;
        ...
        /* loop over each of this module's records and accumulate per fs
         * statistics */
        while(1)
        {
            char *mnt_pt = NULL;
            ...
            if(!fs_type)
                fs_type = "UNKNOWN";
           /* note that it is Ok if we go one over the mount counnt; this is
            * where we will store "unkown" elements
            */
            mnt_pts_stats_index = j;

            if(i == DARSHAN_POSIX_MOD)
            {
                pfile = (struct darshan_posix_file*)mod_buf;
                if(pfile->counters[POSIX_BYTES_READ] > 0)
                    mnt_pts_stats[mnt_pts_stats_index].cp_bytes_read +=
                     pfile->counters[POSIX_BYTES_READ];
                if(pfile->counters[POSIX_BYTES_WRITTEN] > 0)
                    mnt_pts_stats[mnt_pts_stats_index].cp_bytes_written +=
                     pfile->counters[POSIX_BYTES_WRITTEN];
            }
            else if(i == DARSHAN_STDIO_MOD)
            {
                sfile = (struct darshan_stdio_file*)mod_buf;
                if(sfile->counters[STDIO_BYTES_READ] > 0)
                    mnt_pts_stats[mnt_pts_stats_index].cp_bytes_read +=
                     sfile->counters[STDIO_BYTES_READ];
                if(sfile->counters[STDIO_BYTES_WRITTEN] > 0)
                    mnt_pts_stats[mnt_pts_stats_index].cp_bytes_written +=
                     sfile->counters[STDIO_BYTES_WRITTEN];
            }
    ...
    for(i=0; i<mount_count; i++)
    {
        if(mnt_pts_stats[i].cp_bytes_read || mnt_pts_stats[i].cp_bytes_written)
         {
            printf("%s\tCP_BYTES_READ\t%" PRId64 "\n", mnt_data_array[i].mnt_path, mnt_pts_stats[i].cp_bytes_read);
            printf("%s\tCP_BYTES_WRITTEN\t%" PRId64 "\n", mnt_data_array[i].mnt_path, mnt_pts_stats[i].cp_bytes_written);
         }
     }
