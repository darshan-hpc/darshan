...
int main(int argc, char **argv)
{
    ...
    tmp_time += job.start_ts.tv_sec;
    printf("# start_time_asci: %s", ctime(&tmp_time));
    printf("# end_time: %" PRId64 "\n", job.end_ts.tv_sec);
    tmp_time = 0;
    tmp_time += job.end_ts.tv_sec;
    printf("# end_time_asci: %s", ctime(&tmp_time));
    printf("# nprocs: %" PRId64 "\n", job.nprocs);
    run_time = job.end_ts.tv_sec + (job.end_ts.tv_nsec / 1e9);
    run_time -= job.start_ts.tv_sec + (job.start_ts.tv_nsec / 1e9);
    printf("# run time: %.4lf\n", run_time);
