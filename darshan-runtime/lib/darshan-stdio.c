...
#define STDIO_RECORD_OPEN(__ret, __path, __tm1, __tm2, __ts1, __ts2) do { \
    ...
    _STDIO_RECORD_OPEN(__ret, __rec_ref, __tm1, __tm2, __ts1, __ts2, 1, 0); \
    ...
    if(!dC.ldms_lib)\
        if(!dC.stdio_enable_ldms){\
            ...
            darshan_ldms_connector_send(__rec_ref->file_rec->counters[STDIO_OPENS], "open", 0, 0, 0, 0, 0, __tm1, __tm2, __ts1, __ts2, __rec_ref->file_rec->fcounters[STDIO_F_META_TIME], "STDIO", "MET");\
            }\
} while(0)
...
#define STDIO_RECORD_READ(__fp, __bytes,  __tm1, __tm2, __ts1, __ts2) do{ \
    ...
    if(!dC.ldms_lib)\
        if(!dC.stdio_enable_ldms) \
            darshan_ldms_connector_send(rec_ref->file_rec->counters[STDIO_READS], "read", this_offset, __bytes, rec_ref->file_rec->counters[STDIO_MAX_BYTE_READ], 0, 0, __tm1, __tm2, __ts1, __ts2, rec_ref->file_rec->fcounters[STDIO_F_READ_TIME],"STDIO", "MOD"); \
} while(0)
...
#define STDIO_RECORD_WRITE(__fp, __bytes,  __tm1, __tm2, __ts1, __ts2, __fflush_flag) do{ \
    ...
    if(!dC.ldms_lib)\
        if(!dC.stdio_enable_ldms)\
            darshan_ldms_connector_send(rec_ref->file_rec->counters[STDIO_WRITES], "write", this_offset, __bytes, rec_ref->file_rec->counters[STDIO_MAX_BYTE_WRITTEN], 0, rec_ref->file_rec->counters[STDIO_FLUSHES], __tm1, __tm2, __ts1, __ts2,  rec_ref->file_rec->fcounters[STDIO_F_WRITE_TIME], "STDIO", "MOD"); \
} while(0)
...
int DARSHAN_DECL(fclose)(FILE *fp)
{
    ...
    extern struct darshanConnector dC;
    if(!dC.stdio_enable_ldms)
        darshan_ldms_connector_send(-1, "close", 0, 0, 0, 0, rec_ref->file_rec->counters[STDIO_FLUSHES], tm1, tm2, ts1, ts2, rec_ref->file_rec->fcounters[STDIO_F_META_TIME], "STDIO", "MOD");
#endif
