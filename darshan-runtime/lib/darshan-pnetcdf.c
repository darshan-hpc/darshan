...
#define PNETCDF_FILE_PRE_RECORD() do { \
    ...
    if(!darshan_core_disabled_instrumentation()) { \
        if(!pnetcdf_file_runtime) pnetcdf_file_runtime_initialize(); \
        if(pnetcdf_file_runtime) break; \
    } \
    PNETCDF_UNLOCK(); \
    return(ret); \
} while(0)
...
#define PNETCDF_VAR_PRE_RECORD() do { \
    PNETCDF_LOCK(); \
    if(!darshan_core_disabled_instrumentation()) { \
        if(!pnetcdf_var_runtime) pnetcdf_var_runtime_initialize(); \
        if(pnetcdf_var_runtime) break; \
    } \
    PNETCDF_UNLOCK(); \
    return(ret); \
} while(0)
...
static void pnetcdf_var_runtime_initialize()
{
    ...
    /* try and store the default number of records for this module */
    pnetcdf_buf_size = DARSHAN_DEF_MOD_REC_COUNT * sizeof(struct darshan_pnetcdf_var);
    ...
    /* return if darshan-core does not provide enough module memory */
    if(pnetcdf_buf_size < sizeof(struct darshan_pnetcdf_var))
    {
