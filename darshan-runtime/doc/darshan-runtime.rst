...
AutoPerf offers an additional Darshan instrumentation module that may be
...
Users can request Darshan to build the APMPI module by passing
``--enable-apmpi-mod`` option to configure.

The only prerequisite for the APMPI module is that Darshan be configured with a
MPI-3 compliant compiler.
